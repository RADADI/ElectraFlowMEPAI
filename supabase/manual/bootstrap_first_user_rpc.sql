-- ============================================================================
-- ElectraFlow AI — First-user bootstrap RPC (manual patch)
-- ============================================================================
-- Run once in Supabase SQL Editor when organizations/profiles stay empty after
-- Clerk signup. Requires clerk_user_id on profiles (migration-phase5.sql).
--
-- Safe model:
--   • SECURITY DEFINER — creates org/profile/member inside the function
--   • Caller must present Clerk JWT (auth.jwt()->>'sub' = p_clerk_user_id)
--   • First tenant only — fails if any organization already exists
--   • No service_role key; GRANT EXECUTE to authenticated only
-- ============================================================================
...
-- Verify:
-- select proname from pg_proc where proname = 'bootstrap_first_user';
