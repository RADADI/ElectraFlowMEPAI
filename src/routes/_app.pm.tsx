          ...
          <CardContent className="space-y-3">
            {milestoneTimeline.map((m, i) => (
              <div key={i} className="flex items-center gap-3 p-2 rounded-md border">
                <div className="h-9 w-9 rounded-md bg-primary/10 text-primary grid place-items-center text-xs font-semibold">
                  {m.date.split(" ")[1]}
                </div>
              ...
              {projects.map((p) => {
                ...
                return (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="px-3">{formatMoney(p.contract)}</TableCell>
                    <TableCell className="px-3">{formatMoney(p.budget)}</TableCell>
                    <TableCell className="px-3">{formatMoney(p.actualCost)}</TableCell>
                    <TableCell className="px-3 font-medium">{margin}%</TableCell>
                    ...
                    <TableCell className="px-3 whitespace-nowrap">{p.next}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
